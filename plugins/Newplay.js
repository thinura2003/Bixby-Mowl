...
Bixby.addCommand({pattern: 'song ?(.*)', fromMe: false, desc: Lang.SONG_DESC}, (async (message, match) => { 
        ...
        var reply = await message.client.sendMessage(message.jid,`*Preparing to Download...*`,MessageType.text);

        let title = arama[0].title.replace(' ', '+');

        let timeago = arama[0].ago

        let duranm = arama[0].duration

        let nimaurl = arama[0].url
